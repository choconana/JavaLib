...
2. 分配时存在并发安全问题解决方案
   ...
   2. 按线程划分在不同空间上处理：**本地线程分配缓冲区**(Thread Local Allocation Buffer, TLAB)
      * 处理方式：每个线程在Java堆中预先分配一小块内存，当缓冲区用完后，分配新的缓冲区需要同步锁定。
      * 使用方式：-XX:+/-UseTLAB
...
* 劣势：
  ...
  2. 内存空间碎片化：清除操作会产生大量不连续的内存空间，如果分配大对象，可能因为没有足够大的连续内存而提前触发垃圾收集
...
* 作用：由于引用会频繁更新，因此只对影响引用关系的某些指令生成OopMap，在**特定的位置**上才会进行垃圾收集，减少OopMap数量，降低空间成本

* 安全点的选取：**能让程序长时间执行的特征**的指令，通常是**指令序列的复用**，如方法调用、循环跳转、异常跳转等
* 如何让线程停在最近的安全点：
...
* 异常情况：即出现了黑色对象直接指向白色对象。这种情况需同时满足下面两种条件：
  1. 赋值器**插入**了至少一条从黑色对象到白色对象的新引用
  2. 赋值器**删除**了全部从灰色对象到白色对象的直接或间接引用
...
在Minor GC前，虚拟机必须先检查老年代最大可用的连续空间是否大于新生代所有对象总空间：
...
* Java虚拟机保证一个类的`<clinit>()`方法在多线程环境中被正确地加锁同步
...
* 非虚方法：只要能被invokestatic和invokespecial指令调用的方法，都可以在解析阶段中确定唯一的调用版本，符合条件的有**静态方法**、**私有方法**、**实例构造器**、**父类方法**以及**被final修饰的方法**（尽管使用invokevirtual指令调用）5种，这些方法的调用**会在类加载时把符合引用解析为直接引用**，统称为**非虚方法**，其他方法称为**虚方法**
...
* 定义：所有依赖静态类型来决定方法执行版本的分派动作，都称为静态分派
