...
public class RedBlackTreeOperation {
    ...
    */
    ...
    // 用于获得树的层数
    public static int getTreeDepth(RedBlackTree.RBNode root) {
        
        return root == null ? 0 : (1 + Math.max(getTreeDepth(root.getLeft()), getTreeDepth(root.getRight())));
    }


    private static void writeArray(RedBlackTree.RBNode currNode, int rowIndex, int columnIndex, String[][] res, int treeDepth) {
        // 保证输入的树不为空
...
//        res[rowIndex][columnIndex] = String.valueOf(currNode.getValue()+"-"+(currNode.isColor()?"B":"R")+"");
    ...
    public static void show(RedBlackTree.RBNode root) {
        if (root == null) {
