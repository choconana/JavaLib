...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * Returns a power of two size for the given target capacity. 确保容量大于你给定的值，并且是2的整数次幂
     */
    ...
    final Node<K, V>[] resize() {
        ...
        if (oldCap > 0) {//table扩容过
            if (oldCap >= MAXIMUM_CAPACITY) { //当前table容量大于最大值得时候返回当前table（如果原来的长度就比最大长度大）
                threshold = Integer.MAX_VALUE;
                return oldTab;
            } else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                    oldCap >= DEFAULT_INITIAL_CAPACITY)//如果两倍扩容后小于最大容量并且原来的容量大于默认容量
        ...
        table = newTab;//第一次初始化不会进来
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                Node<K, V> e;
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null)//当前index没有发生hash冲突，直接对2取模，即移位运算hash &（2^n -1）  扩容都是按照2的幂次方扩容，因此newCap = 2^n
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
        ...
        /**
         * Ensures that the given root is the first node of its bin. 确保root节点是tab数组(桶)的第一个节点
         */
                    ...
                    for (TreeNode<K, V> p = root; ; ) {//遍历二叉树寻找插入位置
                        ...
                        if ((ph = p.hash) > h)//以下根据hash值计算x插入的位置
                            ...
                            dir = 1;
                                ...
                                (kc = comparableClassFor(k)) == null) ||
                                (dir = compareComparables(kc, k, pk)) == 0)
                            dir = tieBreakOrder(k, pk);//hash值相等 通过tieBreakOrder 计算dir

                        TreeNode<K, V> xp = p;
                        if ((p = (dir <= 0) ? p.left : p.right) == null) {//如果dir<0 树的左节点为空 就将数据插到左边
                            x.parent = xp;
                            if (dir <= 0)
                                xp.left = x;
                            else
        ...
        static <K, V> TreeNode<K, V> rotateLeft(TreeNode<K, V> root,
            ...
            if (p != null && (r = p.right) != null) {//判断p的右节点是否存在 并且给r赋值
                ...
                if ((pp = r.parent = p.parent) == null)//判断p的父节点是否存在 并且p的父节点赋值给r的父节点
                    (root = r).red = false;//如果p的父节点为空 则r变黑色 并赋给root节点
        ...
        static <K, V> TreeNode<K, V> rotateRight(TreeNode<K, V> root,
            ...
            if (p != null && (l = p.left) != null) {//p的左节点是否存在  并给l赋值
                if ((lr = p.left = l.right) != null)//判断l的右节点是否存在 并且 把l的右节变为p的左节点 给lr赋值
                    lr.parent = p;//p变为lr的父节点
                if ((pp = l.parent = p.parent) == null)//判断p的父节点是否存在  把p的父节点变为l的父节点 并给pp赋值
                    (root = l).red = false;//如果不存在 l节点变为黑色 并赋给root节点
        ...
        static <K, V> TreeNode<K, V> balanceInsertion(TreeNode<K, V> root,
                                                      TreeNode<K, V> x) {
            x.red = true;//插入节点m默认为红色
            for (TreeNode<K, V> xp, xpp, xppl, xppr; ; ) {
                if ((xp = x.parent) == null) {//x的父节点如果不为空      插入的为根节点，则直接把颜色改成黑色即可
                    x.red = false;//x变为黑色
                    return x;
                } else if (!xp.red || (xpp = xp.parent) == null)//xp(x的父节点是黑色) 或者 xp的父节点为空(插入的节点的祖父节点为null，即插入的节点的父节点是根节点，直接插入即可（因为根节点肯定是黑色）)
                    return root;//插入的节点的父节点是黑色节点，则不需要调整，因为插入的节点会初始化为红色节点，红色节点是不会影响树的平衡的
                if (xp == (xppl = xpp.left)) {//插入的节点父节点和祖父节点都存在，并且其父节点是祖父节点的左节点
                    if ((xppr = xpp.right) != null && xppr.red) {//插入节点的叔叔节点是红色，则将父亲节点和叔叔节点都改成黑色，然后祖父节点改成红色即可
                        xppr.red = false;
                        xp.red = false;
                        xpp.red = true;
                        x = xpp;
                        ...
                        if (xp != null) {//若为左孩子，则将其父节点变成黑色节点，将其祖父节点变成红色节点，然后将其祖父节点右旋
                            xp.red = false;
                            if (xpp != null) {
                                xpp.red = true;
                ...
                } else {//插入的节点父节点和祖父节点都存在，并且其父节点是祖父节点的右节点
                    if (xppl != null && xppl.red) {//插入节点的叔叔节点是红色，则将父亲节点和叔叔节点都改成黑色，然后祖父节点改成红色即可
                        xppl.red = false;
                        xp.red = false;
                        xpp.red = true;
                        x = xpp;
                    } else {//插入节点的叔叔节点是黑色或不存在
                        if (x == xp.left) {//若插入节点是其父节点的左孩子，则将其父节点右旋
                            root = rotateRight(root, x = xp);
                            xpp = (xp = x.parent) == null ? null : xp.parent;
                        }
                        if (xp != null) {//若为右孩子，则将其父节点变成黑色节点，将其祖父节点变成红色节点，然后将其祖父节点左旋
                            xp.red = false;
                            if (xpp != null) {
                                xpp.red = true;
        ...
        static <K, V> TreeNode<K, V> balanceDeletion(TreeNode<K, V> root,
                                                     TreeNode<K, V> x) {
            for (TreeNode<K, V> xp, xpl, xpr; ; ) {//xp x的父节点 xpl 父节点的左节点 xpr 父节点的右孩子
                if (x == null || x == root)//如果x节点为空或者就是根节点 那么直接返回根节点
                    return root;
                else if ((xp = x.parent) == null) {//如果x的父节点为空 也就是说只存在x这一个节点
                    x.red = false;//把x变为黑色 然后返回x
                    return x;
                } else if (x.red) {//如果x是红色 那么把x变为黑色
                    x.red = false;
                    return root;
